...
/* Commit messages */
const readmeMsg = 'Create README - GitLeet';
const discussionMsg = 'Prepend discussion post - GitLeet';
const createNotesMsg = 'Attach NOTES - GitLeet';
        ...
        for (let i = 0; i < scripts.length; i += 1) {
          ...
          if (text.includes('pageData')) {
            ...
            if (text.includes('runtime') && text.includes('memory')) {
              ...
              resultRuntime = slicedText.slice(
            ...
            if (resultRuntime && resultMemory) {
              finalMsg = `Time: ${resultRuntime}, Memory: ${resultMemory} - GitLeet`;
            }

            if (code !== null) {
              if (!finalMsg.includes('Time:')) {
                finalMsg = `Time: N/A, Memory: N/A - GitLeet`;
              }
              setTimeout(() => {
                uploadGit(
...
function parseStats() {
  ...
  return `Time: ${time} (${timePercentile}), Space: ${space} (${spacePercentile}) - GitLeet`;
}
...
function getNotesIfAny() {
  ...
  if (
    ...
    const [notesdiv] = document
      ...
      .getElementsByClassName('CodeMirror-code');
    if (notesdiv) {
      for (let i = 0; i < notesdiv.childNodes.length; i += 1) {
        if (notesdiv.childNodes[i].childNodes.length === 0) {
          // continue
        } else {
...
/* This will create a tick mark before "Run Code" button signalling GitLeet has done its job */
...
chrome.storage.local.get('isSync', (data) => {
  ...
  if (!data || !data.isSync) {
    ...
    chrome.storage.local.set({ isSync: true }, () => {
      // GitLeet Synced to local values
    });
  } else {
    // GitLeet Local storage already synced!
  }
});
